          ...
          <div className="mx-auto grid max-w-[1240px] overflow-hidden rounded-[24px] border border-[#B38846]/35 bg-[#F8F2EB] shadow-[0_22px_65px_rgba(34,50,58,.07)] lg:grid-cols-[1.05fr_1fr]">
            ...
            <div className="flex items-center p-7 sm:p-10 lg:p-12">
              <div className="w-full">
                <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-[#B38846]">Curated UNIKMO</p>
                <h2 className="mt-4 max-w-[520px] font-serif text-[32px] leading-[1.08] sm:text-[42px]">Want the memory without doing the work?</h2>
                <p className="mt-5 max-w-[540px] text-[13px] leading-[1.75] text-[#22323A]/64 sm:text-[14px]">Send us the photos, videos and messages. We’ll curate the story, handle the optional Times Square experience, and deliver your finished UNIKMO.</p>
                <div className="mt-8 grid gap-3 sm:grid-cols-3">
                  {[
                    ['Keep It', 'Curated Memory', 'We professionally assemble your moments into one finished UNIKMO.'],
                    ['Show It', 'Times Square Edition', 'We curate it, show it in Times Square, capture it and preserve the experience.'],
                    ['Share It', 'Extra Keepsakes', 'Give the same finished memory to family, friends or everyone who was part of it.'],
                  ].map(([eyebrow, title, copy]) => <article key={eyebrow} className="rounded-[16px] border border-[#B38846]/25 bg-white/55 p-5 text-center"><div className="mx-auto flex h-10 w-10 items-center justify-center rounded-full border border-[#B38846]/45 text-[#B38846]"><TrustIcon kind={eyebrow === 'Keep It' ? 'lock' : 'quality'} /></div><p className="mt-4 text-[9px] font-semibold uppercase tracking-[0.16em] text-[#B38846]">{eyebrow}</p><h3 className="mt-1 font-serif text-[18px]">{title}</h3><p className="mt-3 text-[11px] leading-[1.6] text-[#22323A]/58">{copy}</p></article>)}
                </div>
                <div className="mt-7 flex flex-wrap items-center gap-5"><a href="/curated-memories#start" className="inline-flex min-h-[46px] items-center justify-center rounded-lg bg-[#B38846] px-6 text-[11px] font-medium text-white transition hover:bg-[#9D773D]">Have Us Create It <span className="ml-2" aria-hidden="true">→</span></a><a href="/curated-memories" className="text-[11px] font-medium text-[#22323A]/68 underline decoration-[#B38846]/55 underline-offset-4">Learn about Curated UNIKMO →</a></div>
                <p className="mt-5 text-[11px] text-[#22323A]/48">Curated is optional. Standard UNIKMO remains self-created and private by design.</p>
              </div>
            </div>
          </div>
        </section>
