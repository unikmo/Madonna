...
interface Code {
  ...
  code: string;
  user: { email: string };
  order: { shopifyOrderId: string; email: string; source?: 'admin' | 'webhook'; tags?: string[] };
  quantity: number;
  deliveryType: string;
  status: string;
  claimedAt?: string;
...
export default function CodesPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {/* Table */}
      <div className="rounded-2xl border border-[#E3DAD0] bg-white shadow-sm overflow-hidden">
        ...
        <table className="w-full min-w-[1180px] table-auto">
          <thead className="bg-[#F5ECE3]">
            <tr>
              ...
              <th className="px-6 py-4 text-left text-sm font-semibold text-[#2D2926]">Generated by</th>
              ...
              <th className="px-6 py-4 text-left text-sm font-semibold text-[#2D2926]">Created</th>
              ...
              >
                <td className="px-6 py-4 whitespace-nowrap">
                  <Link
                    ...
                    className="text-[#2D2926] hover:underline font-mono text-sm whitespace-nowrap"
                  >
                      {code.code}
                    </Link>
                  </td>
                  <td className="px-6 py-4 text-[#2D2926] whitespace-nowrap">{code.user?.email}</td>
                  <td className="px-6 py-4 text-[#2D2926]/75 whitespace-nowrap font-mono text-sm">{code.order?.shopifyOrderId}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    {(() => {
                      const isAdminGenerated =
                        code.order?.source === 'admin' || Boolean(code.order?.tags?.includes('admin_created'));
                      return (
                        <span
                          className={`px-2 py-1 rounded-full text-[11px] ${
                            isAdminGenerated ? 'bg-blue-50 text-blue-700' : 'bg-amber-50 text-amber-700'
                          }`}
                        >
                          {isAdminGenerated ? 'Admin' : 'Webhook'}
                        </span>
                      );
                    })()}
                  </td>
                  <td className="px-6 py-4 text-[#2D2926]/75 whitespace-nowrap">{code.quantity}</td>
                  <td className="px-6 py-4 text-[#2D2926]/75 capitalize whitespace-nowrap">{code.deliveryType}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`px-2 py-1 rounded-full text-[11px] ${
                        code.status === 'claimed'
                  ...
                  <td className="px-6 py-4 text-[#2D2926]/75 whitespace-nowrap">{code.mediaCount}</td>
                  <td className="px-6 py-4 text-[#2D2926]/70 text-sm whitespace-nowrap">
